...
/* loaded from: challenge-0.0.1-SNAPSHOT.jar:BOOT-INF/lib/spring-web-5.1.2.RELEASE.jar:org/springframework/http/codec/json/AbstractJackson2Decoder.class */
public abstract class AbstractJackson2Decoder extends Jackson2CodecSupport implements HttpMessageDecoder<Object> {
    ...
    private Flux<Object> decodeInternal(Flux<TokenBuffer> tokens, ResolvableType elementType, @Nullable MimeType mimeType, @Nullable Map<String, Object> hints) {
        ...
        if (jsonView != null) {
            readerFor = getObjectMapper().readerWithView(jsonView).forType(javaType);
        } else {
            readerFor = getObjectMapper().readerFor(javaType);
        }
        ObjectReader reader = readerFor;
        return tokens.map(tokenBuffer -> {
            try {
                ...
                throw new DecodingException("JSON decoding error: " + ex2.getOriginalMessage(), ex2);
