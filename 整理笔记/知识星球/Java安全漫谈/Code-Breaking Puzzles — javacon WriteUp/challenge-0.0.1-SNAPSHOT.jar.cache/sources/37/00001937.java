...
/* loaded from: challenge-0.0.1-SNAPSHOT.jar:BOOT-INF/lib/spring-boot-2.1.0.RELEASE.jar:org/springframework/boot/context/properties/bind/validation/ValidationBindHandler.class */
public class ValidationBindHandler extends AbstractBindHandler {
    ...
    private void validate(ConfigurationPropertyName name, Object target, Class<?> type) {
        if (target != null) {
            ...
            Arrays.stream(this.validators).filter(validator -> {
                ...
                validator2.validate(target, errors);
            });
            if (errors.hasErrors()) {
    ...
    private BindValidationException getBindValidationException(ConfigurationPropertyName name, BindingResult errors) {
        Set<ConfigurationProperty> boundProperties = (Set) this.boundProperties.stream().filter(property -> {
            return name.isAncestorOf(property.getName());
